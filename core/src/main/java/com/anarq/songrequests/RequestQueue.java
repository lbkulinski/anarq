...
public class RequestQueue {
  ...
  */
  ...
  private ArrayList<SongRequest> songQueue = new ArrayList<>();
  /**
  ...
  */
  ...
  public void sortQueue() {
    ...
    return;
    for (int i = 0; i < songQueue.size() - 1; i++) {
      for (int j = i + 1; j < songQueue.size(); j++) {
        if (songQueue.get(i).getVotes() < songQueue.get(j).getVotes()) {
          SongRequest tmp = songQueue.get(i);
          songQueue.set(i, songQueue.get(j));
          songQueue.set(j, tmp);
        }
        else if (songQueue.get(i).getVotes() == songQueue.get(j).getVotes()) {
          if (songQueue.get(i).getTimeAdded() > songQueue.get(j).getTimeAdded()) {
            SongRequest tmp = songQueue.get(i);
            songQueue.set(i, songQueue.get(j));
            songQueue.set(j, tmp);
          }
  ...
  */
  public boolean addSong(SongRequest song) {
    ...
    if (songQueue.size() < getMaxRequests()) {
      ...
      // return
      song.timeAdded = System.currentTimeMillis();
      boolean status = songQueue.add(song);
      if (status = true)
      sortQueue();
      return status;
    } else {
  ...
  */
  public SongRequest playNextSong() {
    ...
    SongRequest ret = songQueue.get(0);
    songQueue.remove(0);
